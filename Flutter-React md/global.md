...
**Verification workflow:** Run the playground (`apps/playground`, e.g. `http://127.0.0.1:5173/`). For each component, find `<Name>Preview` in `apps/playground/src/App.tsx`, switch **light / dark / elevated** themes when relevant, and ensure the **Playground parity** section in `<name>-base.md` lists every preview row. Cross-check `packages/css/src/<component>.css` so tokens and motion are not contradicted.
...
1. **Identify variants, sizes, and states** from `Flutter-React md/components/<name>/<name>-base.md`.

...
21. [Playground parity](#21-playground-parity)
...
## 21. Playground parity

**Visual source of truth:** run the playground (`apps/playground`, e.g. `http://127.0.0.1:5173/`) and compare with **`ButtonPreview`** in `apps/playground/src/App.tsx`.

| Preview section | Intent |
|-----------------|--------|
| Variants | One **primary** button per variant (`primary`, `secondary`, `inverted`, `ghost`, `link`, `danger`), size `md`. |
| Sizes | **Primary** at each size (`xs` through `xl`). |
| Icon slots | Primary: download + label; continue + **ArrowRight**; danger + trash; secondary with **Plus** + **ChevronRight**. |
| Icon only | **Heart** icon-only for each size; **Search** / **Plus** / **Trash** icon-only at `md` on secondary / ghost / danger. |
| Loading state | Each variant: click toggles loading ~2s; loading dots replace label. |
| Disabled state | Each variant, `disabled`. |
| Full width | Primary + **ArrowRight**; secondary plain; danger + trash — `fullWidth`. |
| Matrix | Full **variant × size** grid (`Label` in each cell). |

**Usage panel (`ButtonUsage`):** confirm / policy card with loading primary CTA, stacked secondary + ghost, link + small danger — documents real layout, not an extra variant matrix.
